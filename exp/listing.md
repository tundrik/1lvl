...
1. скорость вч-й
   ...
   > хз ???
   > **Параллелизм и конкурентность**: Использование горутин и каналов в Go позволяет эффективно использовать
   > многоядерные процессоры и распределять вычисления между ними. Параллельные вычисления могут значительно ускорить выполнение программы.
   > Меньше данных хранить в хипе для снижения нагрузки на gc, стек работает быстрее
...
5. отказоустойчивость
   > 1. **Использование резервирования ресурсов**: Обеспечьте резервирование ресурсов, таких как память,
   >    файловые дескрипторы и соединения с базой данных. Это может >включать в себя использование пулов соединений и других механизмов,
   >    чтобы предотвратить исчерпании ресурсов.
   > 2. **Обработка ошибок и восстановление**: Важно обрабатывать ошибки и предусматривать механизмы восстановления после сбоев.
   >    Это может включать в себя перезапуск или переключение на резервные системы.
   > 3. **Мониторинг и регистрация**: Реализуйте механизмы мониторинга и регистрации, чтобы быстро обнаруживать сбои и анализировать проблемы в работе приложения.
   >    Это поможет быстро реагировать на сбои и их устранение.
   > 4. **Распределенные системы**: Если приложение состоит из нескольких компонентов, обеспечьте отказоустойчивость на уровне распределенных систем.
   >    Это может включать в себя репликацию данных, механизмы обнаружения и восстановления после сбоев и т. д.
   > 5. **Тестирование**: Проведите тщательное тестирование приложения, включая функциональное, модульное, интеграционное и стресс-тестирование.
   >    Это поможет выявить уязвимости и ошибки до запуска в продакшн.
   > 6. **Резервное копирование и восстановление**: Регулярно создавайте резервные копии данных и конфигурации приложения,
   >    чтобы быстро восстанавливаться после сбоев или катастроф.
